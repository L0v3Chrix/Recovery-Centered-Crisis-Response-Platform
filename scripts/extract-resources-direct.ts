...
// Create a temporary file that imports and exports the data
const tempFile = `
...
async function extractResources() {
  try {
    ...
    // Write and execute temp extraction script
    const tempPath = path.join(process.cwd(), 'scripts/temp-extract.js');
    fs.writeFileSync(tempPath, tempFile);

    // Run the extraction
    await execAsync(`node ${tempPath}`);
